...
class FrameInterface {
 public:
  ...
  virtual int32_t getFrameDataSize() const = 0;
...
struct FrameHolder {
  FrameInterface *frame_;

  FrameHolder() : frame_(nullptr) {}
  explicit FrameHolder(FrameInterface *frame) : frame_(frame) {}
  FrameHolder(const FrameHolder &other) : frame_(other.frame_) {
    frame_->acquire();
  }
  ...
  explicit operator FrameInterface *() const { return frame_; }
  FrameInterface *operator->() const { return frame_; }

  FrameHolder clone() const { return FrameHolder(*this); }
  bool isBlocking() const { return frame_->isBlocking(); }
};
...
class FrameData : public FrameInterface, public noncopyable {
 public:
  ...
  int32_t getFrameDataSize() const override;
