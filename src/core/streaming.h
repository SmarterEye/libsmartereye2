...
struct StreamProfileConfig {
  explicit StreamProfileConfig(StreamFormat format = StreamFormat::FORMAT_CUSTOM,
                               StreamType stream = StreamType::STREAM_ANY,
                               int idx = 0,
                               uint32_t width = 0, uint32_t height = 0,
                               uint32_t framerate = 0) :
      format(format), stream(stream), index(idx), width(width), height(height), fps(framerate) {}
